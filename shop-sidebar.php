		...
		<div class="container">
			...
			<div class="row">
				...
				if (!isset($_GET['category'])) {
					...
					$db = new DBController;
					$db->openConnection();
					$select_query = "SELECT product.*, product_images.image1 FROM product JOIN product_images ON product.id = product_images.product_id ;";
					$db->select($select_query);
						...
						while ($row = mysqli_fetch_assoc($result_pro)) {
							...
							echo "
										...
										<div class='product-content'>
											<h4><a href='product-single.php?product=$product_id'>
												$product_title</a></h4>
			...
			";
				...
				if (isset($_GET['category'])) {
					...
					$db->select($select_query);
					$result_pro = $db->runQuery($select_query);
					$num_of_rows = mysqli_num_rows($result_pro);
					if ($num_of_rows == 0) {
